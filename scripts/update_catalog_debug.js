/**
 * OctoMind - Script de Atualização de Catálogo (Versão Debug)
 * Versão: 2.1.0
 */
...
console.log(`   - Repo atual: ${CURRENT_REPO_NAME}`)
...
async function main() {
  try {
    ...
    console.log("✅ Fetch nativo disponível")
    ...
    if (filteredRepos.length > 0) {
      ...
      filteredRepos.forEach((repo, index) => {
        console.log(`   ${index + 1}. ${repo.name} (${repo.language || "N/A"})`)
      })
    }
...
function filterRepositories(repos) {
  console.log(`🔍 Filtrando repositórios (excluindo ${CURRENT_REPO_NAME})...`)
  const filtered = repos.filter(
    (repo) => repo.name.toLowerCase() !== CURRENT_REPO_NAME.toLowerCase() && !repo.archived && !repo.fork,
  )
  console.log(`🔍 Filtrados: ${filtered.length} de ${repos.length} repositórios`)
  return filtered
}
...
async function fetchAllRepositories() {
  ...
  while (hasMorePages) {
    ...
    try {
      ...
      console.log(`📚 Resposta recebida: ${repos.length} repositórios`)
...
async function updateReadme(repos) {
  try {
    ...
    console.log(`📋 Tabela gerada com ${repos.length} repositórios`)
    ...
    console.log(`📊 Estatísticas: ${repoCount} repos, ${languageCount} linguagens`)
